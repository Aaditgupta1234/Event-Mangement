...
const SAT = {
  ...
  demoUsers: [
    {id: 'demo-1', name: 'Alex Chen', email: 'alex@example.com', role: 'participant'},
    {id: 'demo-2', name: 'Priya Sharma', email: 'priya@example.com', role: 'participant'},
    {id: 'demo-3', name: 'Mike Johnson', email: 'mike@example.com', role: 'participant'},
    {id: 'demo-4', name: 'Sarah Williams', email: 'sarah@example.com', role: 'participant'},
    {id: 'demo-5', name: 'David Lee', email: 'david@example.com', role: 'participant'},
    {id: 'demo-6', name: 'Emma Wilson', email: 'emma@example.com', role: 'participant'},
    {id: 'demo-7', name: 'James Brown', email: 'james@example.com', role: 'participant'},
    {id: 'demo-8', name: 'Lisa Garcia', email: 'lisa@example.com', role: 'participant'}
  ]
};
...
const state = {
  ...
  leaderboardFilter: 'all', // Leaderboard filter: 'all', 'week', 'today'
};
...
const auth = {
  ...
  init() {
    ...
    if (this.users.length === 0) {
      ...
      SAT.demoUsers.forEach((demoUser) => {
        this.users.push({...demoUser, password: 'demo123'});
        store.set(`sb_user_${demoUser.id}`, {
          name: demoUser.name,
    ...
    if (switchViewBtn) switchViewBtn.addEventListener('click', (e) => { e.preventDefault(); this.switchView(); });
  ...
  login() {
    ...
    if (user) {
      ...
      state.isAdminView = user.role === 'admin';
      this.updateUI();
      hideAuth();
      toast(`Welcome back, ${user.name}!`);

      const userKey = `sb_user_${user.id}`;
      state.user = store.get(userKey, { name: user.name, xp:0, level:1, plan:[], scans:[], zoneCounts:{}, reels:[] });
      render();
    } else {
  ...
  signup() {
    ...
    store.set('currentUser', this.user);
    state.isAdminView = newUser.role === 'admin';
    this.updateUI();
    hideAuth();
    toast(`Account created! Welcome, ${name}!`);

    state.user = { name: name, xp:0, level:1, plan:[], scans:[], zoneCounts:{}, reels:[] };
  ...
  switchView() {
    if (this.user && this.user.role === 'admin') {
      ...
      state.view = state.isAdminView ? 'admin' : 'home';
      const btn = document.getElementById('switchViewBtn');
      if (btn) btn.textContent = state.isAdminView ? 'Switch to User View' : 'Switch to Admin View';
...
function initChat() {
  ...
  setTimeout(() => {
    addChatBubble("Try asking me about events, your points, navigation, or the leaderboard. Here are some examples:");
    showSuggestions();
  }, 1000);
}
...
function showSuggestions() {
  ...
  suggestionsDiv.innerHTML = `
    ...
    <div class="suggestion" onclick="useSuggestion(this)">Show me the leaderboard</div>
  `;
...
function useSuggestion(element) {
  ...
  if (chatInput) { chatInput.value = text; sendChatMessage(); }
}
...
function generateChatResponse(question) {
  ...
  if (/leaderboard|rank|top|who is winning|competition/.test(q)) {
    const leaderboardData = getLeaderboardData();
    const top3 = leaderboardData.slice(0, 3);
    if (top3.length > 0) {
      const response = `🏆 Current Top 3:\n${top3.map((user, index) => `${index + 1}. ${user.name} - ${user.xp} XP`).join('\n')}\n\nYour rank: ${leaderboardData.findIndex(u => u.id === auth.user?.id) + 1 || 'N/A'}`;
      return response;
    } else return 'No leaderboard data available yet.';
  }
  ...
  if (/events|show|list|what's happening|what events/.test(q)) {
    ...
    } else return `I couldn't find any events matching your query.`;
  }
  ...
  if (/schedule|plan|my plan|what am i doing|what's my schedule/.test(q)) {
    ...
    } else return `Your plan is empty. Add some events from the Events tab!`;
  }
  ...
  if (/rewards|what can i get|redeem|what rewards/.test(q)) {
    ...
    } else return `You need more XP to redeem rewards. Keep scanning QR codes!`;
  }

  if (/help|hi|hello|hey|how are you|good morning|good afternoon|good evening/.test(q)) {
    const userName = auth.user ? auth.user.name : 'there';
    const greetings = [
      `Hey ${userName}! I'm FestifyXR 👋 Try "events for music", "my points", "navigate to Main Stage", or "show me the leaderboard".`,
      `Hello! I'm here to help with your festival experience.`,
      `Hi there! Ask me about events, points, navigation, your schedule, or the leaderboard.`
    ];
...
- Viewing the leaderboard and rankings
...
- And much more! Just ask me anything about the festival.`;
  ...
  const defaultResponses = [
    `I'm not sure how to respond to that. Try asking about events, points, navigation, or the leaderboard.`,
    `I'm still learning! Try "my points", "events for music", "navigate to Main Stage", or "show me the leaderboard".`,
...
function switchAuthTab(tab) {
  ...
  if (tab === 'login') {
    ...
    if (authFooterText) authFooterText.innerHTML = 'Don\'t have an account? <a href="#" onclick="switchAuthTab(\'signup\')">Sign up</a>';
  } else {
    if (loginForm) loginForm.classList.remove('active');
    if (signupForm) signupForm.classList.add('active');
    if (loginTab) loginTab.classList.remove('active');
    if (signupTab) signupTab.classList.add('active');
    if (authFooterText) authFooterText.innerHTML = 'Already have an account? <a href="#" onclick="switchAuthTab(\'login\')">Login</a>';
  }
}
...
function selectRole(role) {
  ...
  roleOptions.forEach(option => {
    if (option.dataset.role === role) option.classList.add('selected');
...
function toast(msg, ms=2200){ const t=qs('#toast'); if(!t) return; t.textContent=msg; t.style.display='block'; setTimeout(()=>t.style.display='none', ms); }
function setXP(delta){
  state.user.xp = Math.max(0, (state.user.xp||0) + delta);
  const level = Math.floor(1 + state.user.xp / 100);
  state.user.level = level;
  if (auth.user) store.set(`sb_user_${auth.user.id}`, state.user);
  const xpFoot = qs('#xpFoot'); if (xpFoot) xpFoot.textContent = state.user.xp;
}
function greet(){
...
function getLeaderboardData() {
  const participants = auth.users.filter(u => u.role === 'participant');
  const leaderboard = participants.map(user => {
    const userData = store.get(`sb_user_${user.id}`, { xp: 0, level: 1 });
    return {
      id: user.id,
      name: user.name,
      email: user.email,
      ...
      eventsPlanned: userData.plan ? userData.plan.length : 0,
      scansCount: userData.scans ? userData.scans.length : 0
    };
  });
  leaderboard.sort((a, b) => b.xp - a.xp);
  if (state.leaderboardFilter === 'week') return leaderboard.slice(0, Math.floor(leaderboard.length * 0.7));
  if (state.leaderboardFilter === 'today') return leaderboard.slice(0, 10);
  return leaderboard;
}
...
const VIEWS = {
      ...
      <section class="card">
        ...
        <div class="body">
          <ul>
            ...
            <li>🏆 Check out the <a class="inline" href="#" onclick="goto('leaderboard')">Leaderboard</a> to see top performers!</li>
          </ul>
        </div>
      </section>
    </div>
  `;},
  events(){ /* same rendering logic as original HTML */ 
    ...
    <div class="grid grid-2">
      <section class="card">
        ...
        <div class="body">
          ...
          <div class="inputbar">
            <input id="chatInput" placeholder="Ask me anything: 'Where is Robo Wars?' 'What's my XP?' 'Show me the leaderboard'" onkeydown="if(event.key==='Enter') sendChatMessage()">
          ...
          <div class="row muted">
            ...
            <span class="tag">"show me the leaderboard"</span>
          </div>
        </div>
      </section>
      <section class="card">
        ...
        <div class="body list">
          ...
          <div class="item"><div class="meta"><b>View Leaderboard</b><span class="muted">See top performers</span></div><button class="btn alt" onclick="goto('leaderboard')">View</button></div>
        </div>
      </section>
    </div>
  `;},
  ...
  leaderboard(){ 
    const leaderboardData = getLeaderboardData();
    const currentUserRank = leaderboardData.findIndex(u => u.id === auth.user?.id) + 1;
    return `
      <section class="card">
        <div class="leaderboard-header">
          <h3>🏆 Leaderboard</h3>
          <div class="filter-tabs">
            <button class="filter-tab ${state.leaderboardFilter === 'all' ? 'active' : ''}" onclick="setLeaderboardFilter('all')">All Time</button>
            <button class="filter-tab ${state.leaderboardFilter === 'week' ? 'active' : ''}" onclick="setLeaderboardFilter('week')">This Week</button>
            <button class="filter-tab ${state.leaderboardFilter === 'today' ? 'active' : ''}" onclick="setLeaderboardFilter('today')">Today</button>
          </div>
        </div>
        <div class="body">
              ...
              return `
                <div class="leaderboard-item ${isCurrentUser ? 'current-user' : ''}">
                  <div class="rank-badge ${rankClass}">
                    ${rankIcon}
                    ${rank <= 3 ? `<span class="leaderboard-crown">👑</span>` : ''}
                  </div>
                  <div class="leaderboard-user-info">
                    ...
                    <div class="leaderboard-details">
                      ...
                      <span>•</span>
                      <span>${user.eventsPlanned} events</span>
                      <span>•</span>
                      <span>${user.scansCount} scans</span>
                    </div>
                  </div>
          ...
          ${currentUserRank > 0 ? `
            <div style="margin-top: 20px; padding: 16px; background: rgba(106, 242, 198, 0.1); border-radius: 12px; border: 1px solid rgba(106, 242, 198, 0.2);">
              <div style="text-align: center;">
                <div style="font-size: 14px; color: var(--sub); margin-bottom: 4px;">Your Current Rank</div>
                <div style="font-size: 24px; font-weight: bold; color: var(--brand-2);">#${currentUserRank}</div>
                <div style="font-size: 14px; color: var(--sub); margin-top: 4px;">Keep earning XP to climb higher!</div>
              </div>
            </div>
          ` : ''}
        </div>
    ...
    return `
      <div class="grid grid-2">
        ...
        <section class="card">
          ...
          <div class="body list">
            ${getLeaderboardData().slice(0, 5).map(user => `
              <div class="item">
                <div class="meta">
                  <b>${user.name}</b>
                ...
                <div class="row">
                  <span class="pill">XP: ${user.xp}</span>
                  <span class="pill">Level ${user.level}</span>
                </div>
              </div>
            `).join('')}
          </div>
...
function goto(view){
  if (view === 'admin' && (!auth.user || auth.user.role !== 'admin' || !state.isAdminView)) return;
  state.view = view;
  qsa('#nav button').forEach(b => b.classList.toggle('active', b.dataset.view===view));
  render();
  if (view === 'buddy') setTimeout(() => initChat(), 300);
}
...
/* ========= FEATURES (event plan, nav, checkin) ========= */
...
function recommend(){
  return SAT.events.filter(e=>!state.user.plan.includes(e.id)).sort((a,b)=>b.points-a.points).slice(0,3);
}
...
function promptCode(){ const code = prompt("Enter QR code:"); if(code) processQR(code.trim()); }
function processQR(code){
  ...
  else if(entry.type==='spot'){ ensureZoneCounts(); state.user.zoneCounts['z2'] += 8; store.set('sb_user', state.user); }
...
function showZoneInfo(zoneId) {
  ...
  if (zoneInfo) zoneInfo.innerHTML = infoHTML;
...
function toggleVoice(){
  ...
  if(!listening){ recog.start(); listening=true; updateVoiceBtn(); toast('Listening…'); }
  else { recog.stop(); listening=false; updateVoiceBtn(); }
}
function updateVoiceBtn(){
  const b=qs('#voiceBtn'); if(!b) return; b.textContent = listening?'⏹️ Stop':'🎙️ Voice';
}
...
function previewReel(){
  const canvas = qs('#reelCanvas'); if(!canvas) return;
...
function resetDemo(){
  if (auth.user) {
    state.user = { name: auth.user.name, xp:0, level:1, plan:[], scans:[], zoneCounts:{}, reels:[] };
...
function exportUserData() {
  const data = { users: auth.users, userData: {} };
  auth.users.forEach(u => { data.userData[u.id] = store.get(`sb_user_${u.id}`, {}); });
  const blob = new Blob([JSON.stringify(data, null, 2)], {type: 'application/json'});
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a'); a.href = url; a.download = 'festifyxr-data.json'; a.click();
  toast('Data exported successfully');
}
function resetAllData() {
  if (confirm('Are you sure you want to reset all user data? This cannot be undone.')) {
    auth.users.forEach(u => store.remove(`sb_user_${u.id}`));
    auth.users = [{ id: 'admin-1', name: 'Fest Admin', email: 'admin@festifyxr.com', password: 'admin123', role: 'admin' }];
